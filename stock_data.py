...
def train_test_split_lstm(stocks, prediction_time=1, test_data_size=450, unroll_length=50):
    """
    Splits the data set into training and testing feature for Long Short Term Memory Model.
    """
    test_data_cut = test_data_size + unroll_length + 1

    # Ensure proper slicing without negative indices
    x_train = stocks[0:-prediction_time - test_data_cut].to_numpy()  # Convert to numpy array
    y_train = stocks[prediction_time:-test_data_cut]['Close'].to_numpy()  # Extract 'Close' and convert to array

    x_test = stocks[-test_data_cut:-prediction_time].to_numpy()  # Convert to numpy array
    y_test = stocks[prediction_time - test_data_cut:]['Close'].to_numpy()  # Extract 'Close' and convert to array
